...
export default function OwnerHeader({ title }: OwnerHeaderProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        ...
        const name: string = u?.name || u?.username || u?.email || 'Owner';
        setUserName(name);
        return;
      }
  ...
  const roleLabel = language === 'ja' ? 'オーナー' : 'Chủ nhà hàng';
