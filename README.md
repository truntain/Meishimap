...
> **MESHIMAP** là một nền tảng tìm kiếm và đặt bàn nhà hàng hỗ trợ tiếng Nhật tại Việt Nam
...
*   **Đánh giá & Phản hồi (Reviews)**: Viết đánh giá bằng tiếng Nhật/tiếng Việt kèm điểm số (1-5 sao) và báo cáo vi phạm đánh giá không chính xác qua biểu tượng cờ `🚩` trực tiếp trên trang chi tiết.
*   **Đặt bàn trực tuyến (Online Table Booking)**: Đặt bàn nhanh chóng trực tiếp trên trang chi tiết nhà hàng. Hệ thống tích hợp **Socket.io** cho phép theo dõi trạng thái đơn hàng thời gian thực ngay trên modal (Chờ duyệt `⏳` -> Đã nhận `✅` / Từ chối `❌` kèm lý do cụ thể từ chủ quán) mà không cần chuyển hướng trang.
...
*   **Quản lý đặt bàn (Booking Dashboard)**: Tiếp nhận yêu cầu đặt bàn theo thời gian thực, duyệt (Approve) hoặc từ chối (Reject) kèm lý do chi tiết gửi tới khách hàng. Hỗ trợ **Xóa (Delete)** các đơn đặt bàn đã xử lý xong (chấp nhận/từ chối) để dọn dẹp danh sách quản lý.
...
*   **Kiểm duyệt & Quản lý nhà hàng**: Phê duyệt hoặc từ chối hồ sơ đăng ký mới và hồ sơ tài liệu xác thực năng lực tiếng Nhật. Đồng thời cho phép **Xóa nhà hàng** đã xử lý (đã duyệt/đã từ chối) khỏi hệ thống.
*   **Quản lý người dùng**: Khóa/mở khóa tài khoản khách hàng và chủ nhà hàng để bảo vệ môi trường ứng dụng lành mạnh.
*   **Quản lý phản hồi (Moderation)**: Tiếp nhận báo cáo đánh giá vi phạm từ người dùng tại trang quản trị tập trung. Admin có thể chọn **Bỏ qua (Dismiss)** báo cáo để giữ lại đánh giá hoặc **Xóa vĩnh viễn (Delete)** đánh giá khỏi hệ thống (tự động tính toán lại điểm rating trung bình của nhà hàng tương ứng).
...
│   │   ├── (public)/              # Các trang công khai (/login, /register, /search, /, /about)
